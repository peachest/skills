...
# Harbor 已登录
sudo nerdctl login harbor.transwarp.io
```
...
sudo nerdctl --namespace k8s.io build --platform linux/amd64 \
  -f <DOCKERFILE> \
  <BUILD_ARGS> \
  -t harbor.transwarp.io/aip-amd/<IMAGE_NAME>:<IMAGE_TAG> \
  -t harbor.transwarp.io/aip-amd/<IMAGE_NAME>:latest \
  .
```
...
sudo nerdctl --namespace k8s.io build --platform linux/arm64 \
  -f <DOCKERFILE> \
  <BUILD_ARGS> \
  -t harbor.transwarp.io/aip-arm/<IMAGE_NAME>:<IMAGE_TAG> \
  -t harbor.transwarp.io/aip-arm/<IMAGE_NAME>:latest \
  .
```
...
```bash
# amd64
sudo nerdctl --namespace k8s.io push harbor.transwarp.io/aip-amd/<IMAGE_NAME>:<IMAGE_TAG>
sudo nerdctl --namespace k8s.io push harbor.transwarp.io/aip-amd/<IMAGE_NAME>:latest

# arm64
sudo nerdctl --namespace k8s.io push harbor.transwarp.io/aip-arm/<IMAGE_NAME>:<IMAGE_TAG>
sudo nerdctl --namespace k8s.io push harbor.transwarp.io/aip-arm/<IMAGE_NAME>:latest
```
...
for REPO in aip aip-mm; do
  FULL=harbor.transwarp.io/${REPO}/<IMAGE_NAME>:<IMAGE_TAG>
  LATEST=harbor.transwarp.io/${REPO}/<IMAGE_NAME>:latest
  ...
  sudo nerdctl --namespace k8s.io manifest create "$FULL" \
    harbor.transwarp.io/aip-amd/<IMAGE_NAME>:<IMAGE_TAG> \
    harbor.transwarp.io/aip-arm/<IMAGE_NAME>:<IMAGE_TAG>
  ...
  # 可选 minor tag
  if [ -n "<MINOR_TAG>" ]; then
    MINOR=harbor.transwarp.io/${REPO}/<IMAGE_NAME>:<MINOR_TAG>
...
sudo nerdctl --namespace k8s.io manifest inspect \
  harbor.transwarp.io/aip-mm/<IMAGE_NAME>:<IMAGE_TAG> | grep -A2 platform
```
...
- **push 报 unauthorized** — `sudo nerdctl login harbor.transwarp.io` 重新登录。
