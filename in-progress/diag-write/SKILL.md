...
## Step 5 — Commit, then refresh the zg index
...
Then re-index so the new/updated entry is visible to diag-search's zg semantic fallback:

```bash
cd ~/ops && zg index
```

(Incremental, seconds. Skipping it leaves the note invisible to `zvec_grep_search` until someone indexes manually.)
...
- [ ] `zg index` re-run — the entry is visible to zg semantic search
