...
2. **Harbor auth** — `/root/.docker/config.json` contains an `auths` entry for
   `harbor.transwarp.io`. `nerdctl login harbor.transwarp.io` has a known bug
   (`expected acArg to be harbor.transwarp.io:443, got harbor.transwarp.io`),
   even with `:443` appended. Write the config directly:

   ```bash
   echo '{"auths":{"harbor.transwarp.io":{"auth":"'$(echo -n 'USER:PASS' | base64 -w0)'"}}}' \
...
sudo nerdctl build \
  ...
  --build-arg no_proxy=<NO_PROXY> \
  -t harbor.transwarp.io/<project>/<image>:<tag> \
...
```
NO_PROXY=harbor.transwarp.io,localhost,127.0.0.1,<internal-cidr>
```
...
```bash
sudo nerdctl push harbor.transwarp.io/<project>/<image>:<tag>
```
