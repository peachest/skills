...
Type `/skill:codebase-design`, or the agent reaches for it automatically when a design task fits.
...
Known, and filed as [issue #449](https://github.com/mattpocock/skills/issues/449). The skill is model-invoked and describes itself as vocabulary, but nothing in it hard-stops an agent from treating it as a runnable process. Told to "resume in /skill:codebase-design and drive the open decisions", an agent reached for the most action-shaped content it could find — the parallel sub-agents in `DESIGN-IT-TWICE.md` — re-explored code a previous session had already mapped, and ran a long way before asking anything. None of the guardrails a driver skill has (checkpoints, one question at a time, no auto-advance) are present here, because a reference has none. The workaround is to name a driver skill and let this one sit underneath it: `/skill:grill-with-docs`, `/skill:improve-codebase-architecture` or `/skill:tdd` with `codebase-design` as the vocabulary. The issue is open.
...
- Invoking it does not start a session. If the agent begins reading files and proposing refactors off the back of `/skill:codebase-design` alone, it has mistaken the reference for a driver.
