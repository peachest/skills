...
|------|------|--------|----------------|
| S1 position.sh：逐 worktree TSV 采集（upstream 链解析、/usr/bin/git、单调用覆盖全 worktree） | scripts/position.sh | 899ea13 | 冒烟：nodexpu 19 worktree + ppu 9 worktree 全对，detached/双 remote/临时 worktree 均正确 |
| S2 mr-state.sh：glab api MR 状态（host 从 remote 推导，多实例 401 免疫；blocking 字段提取 Depends on） | scripts/mr-state.sh | 899ea13 | 冒烟：nodexpu !12 open ✓、ppu !25 merged ✓、base-charts open 列表 ✓ |
| S3 frontier.sh：map→open/pending limbo 闭环态（map iid 过滤、Closes 提取） | scripts/frontier.sh | 899ea13 | 冒烟：nodexpu 双 map + pending limbo ✓、ppu #115 pending ✓、base-charts 空输出（正确沉默）✓ |
