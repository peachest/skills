...
## P-007 注入后凭记忆作答（用户直接观察）
- 现象: 执行 reckon 后，agent 未重新查询 git/MR/wayfinder，直接根据上下文与会话记忆拼出"基线"——记忆冒充位置，违反纯重建原则
- 根因: contract gap —— 契约未把"注入触发 = 强制重新采集"写成硬 gate；"Data sources are pure reconstruction" 只是陈述数据源，未禁止记忆作答路径
- 方案: skill-doc——注入触发即强制采集（配合 P-004 脚本一次调用完成）；短路只短输出不短查询：采集结果与上下文一致 → 输出一行"位置未变"；不一致 → 输出 diff。凭记忆作答 = 位置错误
- 证据: 用户观察 2026-09-10（未定位具体 trace）；相邻证据 019fd573#1630（只查 MR+issues 无 git 状态）、01a0435a#2238（注入后纯工具调用零基线文本，无采集）
- 出现: 2026-09-10 diagnose#1 收尾时用户补充
