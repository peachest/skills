...
远程地址含 `gitlab.com` 或自建 GitLab 实例（如 `gitblue.transwarp.io`、`gitlab.transwarp.io`）时判定为 GitLab。

```bash
git remote get-url origin
# git@gitlab.com:ns/proj.git              → GitLab
# git@gitblue.transwarp.io:ns/proj.git    → GitLab (self-hosted)
# http://gitlab.transwarp.io/ns/proj.git  → GitLab (self-hosted)
```
...
同一台机器可能配置多个 GitLab 实例（如 `gitlab.com`、`gitblue.transwarp.io`、`gitlab.transwarp.io`）。脚本按以下优先级解析实例上下文：
...
```bash
glab api --hostname gitlab.transwarp.io \
