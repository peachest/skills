---
name: review-spec
description: Inspect a wayfinder route before handing off to implement — find the gaps and route each to the skill that closes it.
...
Acceptance criteria must cover **every point** the traverse will touch. A route that verifies "discovery works" but never mentions "the mock server must set the new interface method or existing tests panic" has a coverage gap — the traverse will hit the panic and re-open a decision the route should have closed.
...
For each item with no acceptance criterion, report: the item, why the traverse will touch it, and what criterion is missing.
...
For each change the route describes, ask: could two competent travelers read this and take materially different paths? If yes, the route is under-specified. For each under-specified change, report: the passage, the ambiguity, and what the route must specify to make the traverse mechanical.
...
When the verdict is **Held**, the report asks the user which gaps to route:

> Found N gaps. Which should I route to their follow-up skills?
> - [1] research — `/skill:research` (AFK sub-agent)
> - [2] grilling — `/skill:grill-with-docs` (HITL dialogue)
> - [3] prototype — `/skill:prototype` (HITL artifact)
> - [4] task — manual execution
> - …
> Select the gaps to route, or "revise spec" to fix inline instead.
...
**Execution rules** (the parent session follows these after the user chooses):
...
- **grilling** → invoke `/skill:grill-with-docs` in the parent session (HITL — one question at a time). The resolved decision goes into the route.
- **task** → hand the user a precise checklist (HITL) or drive it directly where the agent can (AFK). Feed any resulting facts back into the route.
